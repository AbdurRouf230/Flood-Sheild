...
export default function CampaignHubPage() {
  ...
  const isNgo = mongoUser?.role === 'NGO';
  const isFieldRep = mongoUser?.role === 'NGORepresentative' || mongoUser?.role === 'NGORepLogistics' || mongoUser?.role === 'GovRepLogistics' || mongoUser?.role === 'GovRepresentative';
  const isNgoRep = isFieldRep;
  const isGov = mongoUser?.role === 'Government';

  const [activeTab, setActiveTab] = useState(isNgoRep ? 'my-campaign' : 'campaigns');
  ...
  // Form states (NGO - Create Campaign)
  const [campName, setCampName] = useState('');
  const [campDistrict, setCampDistrict] = useState('Sylhet');
  const [campLat, setCampLat] = useState('24.8950');
  const [campLon, setCampLon] = useState('91.8700');
  const [campCapacity, setCampCapacity] = useState('500');
  const [campPhone, setCampPhone] = useState('');

  // Form states (NGO - Create Invite)
  ...
  const campMapRef = useRef(null);

  // Form states (NGO - Create Logistics Rep Invite)
  ...
  const [campInventoryItems, setCampInventoryItems] = useState([
  ...
  // Form states (NGO - Create Logistics Hub)
  ...
  const fetchData = async (isBackground = false) => {
    ...
    try {
      if (isNgo || isGov) {
        const resC = await fetch(`${API_URL}/campaigns`, { headers: getAuthHeader() });
        if (resC.ok) updateIfChanged(setCampaigns, prevCampRef, await resC.json());
        ...
        if (isNgo) {
          const resI = await fetch(`${API_URL}/campaigns/invites`, { headers: getAuthHeader() });
          if (resI.ok) setInvites(await resI.json());

          const resR = await fetch(`${API_URL}/campaigns/requests`, { headers: getAuthHeader() });
        ...
        const resInv = await fetch(`${API_URL}/campaigns/inventory`, { headers: getAuthHeader() });
        if (resInv.ok) updateIfChanged(setInventory, prevInvRef, await resInv.json());

        const resR = await fetch(`${API_URL}/campaigns/requests/mine`, { headers: getAuthHeader() });
        if (resR.ok) updateIfChanged(setRequests, prevReqRef, await resR.json());
  ...
  // Campaign map picker - Leaflet initialization
  useEffect(() => {
    if (!showCampMap) return;
    const timer = setTimeout(() => {
      const container = document.getElementById('camp-map-container');
      if (!container) return;
      if (campMapRef.current) {
        campMapRef.current.invalidateSize();
        return;
      }
      const initialLat = parseFloat(campLat) || 24.8950;
      const initialLon = parseFloat(campLon) || 91.8700;

      const initMap = () => {
        const map = window.L.map(container, { center: [initialLat, initialLon], zoom: 11 });
        campMapRef.current = map;
        window.L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
          maxZoom: 19, attribution: '&copy; OpenStreetMap contributors'
        }).addTo(map);
        const marker = window.L.marker([initialLat, initialLon], { draggable: true }).addTo(map);
        marker.bindPopup('<b>Campaign Location</b>').openPopup();
        const updateCoords = async (lat, lng) => {
          setCampLat(lat.toFixed(4));
          setCampLon(lng.toFixed(4));
    ...
    return () => {
      clearTimeout(timer);
      if (campMapRef.current) { campMapRef.current.remove(); campMapRef.current = null; }
    };
  }, [showCampMap]);

  // Inventory item helpers
  const addInventoryRow = () => {
    setCampInventoryItems(prev => [...prev, { itemType: 'Water', quantity: '' }]);
  };
  const removeInventoryRow = (idx) => {
    setCampInventoryItems(prev => prev.filter((_, i) => i !== idx));
  };
  const updateInventoryRow = (idx, field, value) => {
    setCampInventoryItems(prev => prev.map((item, i) => i === idx ? { ...item, [field]: value } : item));
  };

  const handleCreateCampaign = async (e) => {
    e.preventDefault();
    if (!campName.trim()) return setError('Campaign name is required.');
    ...
    try {
      ...
      setMessage(`Campaign "${data.name}" created successfully!`);

      // Post inventory stock items to the logistics restock endpoint
      const validItems = campInventoryItems.filter(i => i.quantity && parseInt(i.quantity) > 0);
      if (validItems.length > 0) {
        setAddingStock(true);
        const warehouseName = `${campName} (${campDistrict})`;
      ...
      setCampName(''); setCampPhone('');
      setCampInventoryItems([{ itemType: 'Food', quantity: '' }]);
      fetchData();
    } catch (err) {
  ...
  const handleCreateLogisticsHub = async (e) => {
    ...
    try {
      ...
      // 1. Save Hub Card into Campaign database registry
  ...
  const handleGetCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setCampLat(pos.coords.latitude.toFixed(4));
        setCampLon(pos.coords.longitude.toFixed(4));
        setDetectingLoc(false);
  ...
  const handleCreateInvite = async (e) => {
    e.preventDefault();
    if (!repName.trim() || !selectedCampId) return setError('Representative name and Campaign selection are required.');
    setActionLoading(true);
    setError(''); setMessage('');
    setLastGeneratedInvite(null);
    setEmailStatus('');
    try {
      const res = await fetch(`${API_URL}/campaigns/invite`, {
        method: 'POST',
        headers: getAuthHeader(),
        body: JSON.stringify({ name: repName, email: repEmail.trim(), campaignId: selectedCampId })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Failed to create invite token');
      const selectedCampaign = campaigns.find(c => c.campaignId === selectedCampId);
      setLastGeneratedInvite({
        inviteId: data.inviteId,
        name: repName.trim(),
        campaignName: selectedCampaign?.name || selectedCampId,
        email: repEmail.trim()
      });
  ...
  const handleCreateLogisticsInvite = async (e) => {
    ...
    try {
      const res = await fetch(`${API_URL}/campaigns/logistics-invite`, {
        method: 'POST',
        headers: getAuthHeader(),
  ...
  const handleSendInviteEmail = async () => {
    ...
    setEmailStatus('');
    try {
      await sendInviteEmailRequest('/campaigns/invite/send-email', getAuthHeader(), {
        toEmail: lastGeneratedInvite.email,
        toName: lastGeneratedInvite.name,
        inviteToken: lastGeneratedInvite.inviteId,
        campaignName: lastGeneratedInvite.campaignName
      });
      setEmailStatus('sent');
  ...
  const handleSubmitRepRequest = async (e) => {
    ...
    try {
      ...
      const finalItemsNeeded = `${finalCategory}: ${requestQuantity}`;
      const res = await fetch(`${API_URL}/campaigns/requests`, {
        method: 'POST',
        headers: getAuthHeader(),
        body: JSON.stringify({ itemsNeeded: finalItemsNeeded, urgency })
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Failed to submit request');
      setMessage('Request submitted to parent NGO successfully!');
      setRequestQuantity('');
  ...
  const handleRespondRequest = async (reqId, status) => {
    setActionLoading(true);
    try {
      const res = await fetch(`${API_URL}/campaigns/requests/${reqId}`, {
        method: 'PUT',
        headers: getAuthHeader(),
        body: JSON.stringify({ status, responseNote: `Action taken by ${mongoUser.name}` })
      });
  ...
  const handleDispatchShipment = async (transportId) => {
    ...
    try {
      const res = await fetch(`${API_URL}/transport/${transportId}/status`, {
        method: 'PUT',
        headers: getAuthHeader(),
        body: JSON.stringify({ status: 'In Transit', dispatchedByText: `${mongoUser?.name || 'NGO'} (${mongoUser?.assignedHub ? 'Logistics' : 'NGO Admin'})` })
      });
      const data = await res.json();
  ...
  const handleLoadCar = async (transportId, vr = null) => {
    ...
    try {
      ...
      // If transport object was not pre-created, create it now
      if (!targetTransportId && vr) {
        const createRes = await fetch(`${API_URL}/transport`, {
          ...
          body: JSON.stringify({
            ...
            origin: mongoUser?.assignedHub || mongoUser?.campaignName || 'Logistics Depot',
            destination: vr.villageName || 'Relief Location',
            itemsSummary: `${vr.quantity} ${vr.itemType}`,
            assignedHub: mongoUser?.assignedHub || mongoUser?.campaignName || 'Logistics Depot',
      ...
      if (targetTransportId) {
        const res = await fetch(`${API_URL}/transport/${targetTransportId}/load`, {
          method: 'PUT',
          headers: getAuthHeader(),
          body: JSON.stringify({ loadStatus: 'Loaded', assignedHub: mongoUser?.assignedHub || mongoUser?.campaignName || 'Logistics Depot' })
        });
  ...
  const handleDispatchVillageRequest = async (vr) => {
    ...
    try {
      const response = await fetch(`${API_URL}/logistics/allocate`, {
        ...
        body: JSON.stringify({
          requestId: vr._id,
          warehouseName: mongoUser.assignedHub || mongoUser.campaignName || 'NGO Campaign Hub',
          allocatedItems: [
            { itemType: vr.itemType, quantity: vr.quantity }
          ],
          routeDistance: 45.0,
          routePath: [mongoUser.assignedHub || mongoUser.campaignName || 'NGO Campaign Hub', vr.villageName],
          dispatchedByText: `${mongoUser.name} (${mongoUser.assignedHub ? 'Logistics' : 'Campaign Rep'})`
        })
      });
      if (!response.ok) {
  ...
  // Incoming shipments whose destination is this rep's hub, campaign, or shelter only
  const myTransports = transports.filter(t => {
    const campaignName = (mongoUser?.campaignName || '').trim();
    const assignedHub = (mongoUser?.assignedHub || '').trim();
    const myPlace = assignedHub || campaignName;
  ...
  const isLogisticsRepUser = mongoUser?.role === 'NGORepLogistics' || (mongoUser?.role === 'NGORepresentative' && !!mongoUser?.assignedHub);

  const dispatchReadyTransports = transports.filter((t) => {
    if (t.loadStatus !== 'Loaded') return false;
    if (t.status && t.status !== 'Pending') return false;
    if (!isLogisticsRepUser) return false;
    const hub = (mongoUser?.assignedHub || '').trim();
    if (!hub) return false;
    return facilityMatch(t.origin, hub) || facilityMatch(t.assignedHub, hub);
  });
  ...
  const renderDispatchList = () => (
    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4">
      ...
      {dispatchReadyTransports.length === 0 ? (
        <div className="py-10 text-center text-slate-500 text-sm">
          <Package className="w-8 h-8 mx-auto mb-2 opacity-30" />
          No loaded vehicles waiting to dispatch.
        </div>
      ) : (
        <div className="space-y-3">
          {dispatchReadyTransports.map((t) => (
            <div key={t._id} className="bg-slate-900 rounded-xl border border-slate-800 p-4 flex flex-col sm:flex-row justify-between gap-3">
              <div className="space-y-1.5 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold border bg-amber-500/10 text-amber-400 border-amber-500/20">
                    {t.status || 'Pending'}
                  </span>
                  <span className="px-2 py-0.5 rounded text-[10px] font-bold border bg-emerald-500/10 text-emerald-300 border-emerald-500/20">
                    📦 Loaded
                  </span>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 p-6 md:p-10">
      <div className="max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">
                  {mongoUser?.assignedHub ? 'NGO Logistics Hub' : 'NGO Campaign Hub'}
                </h1>
                <p className="text-slate-400 text-sm">
                  {isNgo && `Parent Organization: ${mongoUser?.orgName || mongoUser?.name}`}
                  {isNgoRep && (mongoUser?.assignedHub ? `Assigned Hub: ${mongoUser?.assignedHub} (${mongoUser?.district})` : `Assigned Campaign: ${mongoUser?.campaignName} (${mongoUser?.district})`)}
                  {isGov && 'Government Read-Only Overview of Active NGO Relief Camps'}
                </p>
              </div>
            </div>
          </div>
        ...
        {isNgo && (
          <div className="flex border-b border-slate-800 gap-6">
            <button
              ...
              className={`pb-4 font-semibold text-sm transition flex items-center gap-2 border-b-2 ${activeTab === 'campaigns' ? 'border-emerald-400 text-emerald-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
            >
              <Tent className="w-4 h-4" /> Active Campaigns ({campaigns.length})
            </button>
            <button
              onClick={() => setActiveTab('invites')}
              className={`pb-4 font-semibold text-sm transition flex items-center gap-2 border-b-2 ${activeTab === 'invites' ? 'border-emerald-400 text-emerald-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
            >
            ...
            <button
              ...
              className={`pb-4 font-semibold text-sm transition flex items-center gap-2 border-b-2 ${activeTab === 'requests' ? 'border-emerald-400 text-emerald-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                }`}
            >
              <Send className="w-4 h-4" /> Campaign Requests ({requests.length})
            </button>
          </div>
        )}

        {/* NGO VIEW - TAB 1: CAMPAIGNS */}
        {(isNgo && activeTab === 'campaigns') && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Create Campaign / Logistics Hub Form Container */}
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-5">
              {/* Option Selector: Relief Campaign vs Logistics Hub */}
              <div className="flex bg-slate-900/90 p-1.5 rounded-xl border border-slate-800 gap-1.5">
                <button
                  type="button"
                  onClick={() => setCreationMode('campaign')}
                  className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer ${creationMode === 'campaign'
                      ? 'bg-emerald-500 text-slate-950 shadow-md font-extrabold'
                      : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
                    }`}
                >
                  <Tent className="w-4 h-4" /> Relief Campaign
                </button>
                <button
                  type="button"
                  onClick={() => setCreationMode('hub')}
                  className={`flex-1 py-2 px-3 rounded-lg text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer ${creationMode === 'hub'
              ...
              {creationMode === 'campaign' && (
                <div className="space-y-4 animate-fade-in">
                  <div className="flex items-center gap-2 text-emerald-400 font-bold text-base border-b border-slate-900 pb-2">
                    <PlusCircle className="w-4 h-4" /> Launch New Relief Campaign
                  </div>
                  <form onSubmit={handleCreateCampaign} className="space-y-4">
                    ...
                    <div className="space-y-2 bg-slate-900/60 p-3 rounded-xl border border-slate-800">
                      <div className="flex justify-between items-center">
                        <label className="block text-xs font-medium text-slate-400 flex items-center gap-1.5">
                          <MapPin className="w-3.5 h-3.5 text-emerald-400" /> Campaign Coordinates
                        </label>
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            {showCampMap ? 'Hide Map' : 'Pick on Map'}
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <input
                            type="text"
                            value={campLon}
                            onChange={(e) => setCampLon(e.target.value)}
                            className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-emerald-500"
                          />
                        </div>
                      </div>
                      ...
                      <div
                        ...
                        style={{ height: showCampMap ? '260px' : '0px', display: showCampMap ? 'block' : 'none' }}
                        className="w-full rounded-xl border border-sky-500/30 relative"
                      ></div>
                      {showCampMap && (
                        <p className="text-[10px] text-slate-500 text-center italic">Click or drag marker on map to set campaign location</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2 bg-slate-900/60 p-3 rounded-xl border border-emerald-500/20">
                      <div className="flex justify-between items-center">
                        <label className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                          <Boxes className="w-3.5 h-3.5" /> Initial Campaign Stock
                        </label>
                        <button
                          type="button"
                      ...
                      <div className="space-y-2">
                        {campInventoryItems.map((item, idx) => (
                    ...
                    >
                      {actionLoading ? 'Creating Campaign...' : addingStock ? 'Adding Stock...' : (
                        <><Tent className="w-4 h-4" /> Launch Campaign</>
                      )}
                    </button>
                  </form>
                </div>
            ...
            <div className="lg:col-span-2 space-y-4">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <Tent className="w-5 h-5 text-emerald-400" /> Active NGO Relief Campaigns & Logistics Hubs
              </h2>
              {campaigns.length === 0 ? (
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-8 text-center text-slate-400 text-sm">
                  No active campaigns or logistics hubs found. Use the form to launch one.
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {campaigns.map(c => {
                    ...
                    return (
                      <div key={c._id || c.campaignId} className={`bg-slate-950 border rounded-2xl p-5 space-y-3 ${isHubCard ? 'border-sky-500/30' : 'border-slate-800'}`}>
                        ...
                        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-900 text-xs text-slate-300">
                          <div><span className="text-slate-500">ID:</span> {c.campaignId}</div>
        ...
        {/* NGO VIEW - TAB 2: INVITES */}
        {(isNgo && activeTab === 'invites') && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="space-y-4">
              {/* Toggle: Campaign Rep vs Logistics Hub Rep */}
              <div className="flex gap-2 p-1 bg-slate-950 border border-slate-800 rounded-xl">
                <button
                  onClick={() => setCreationMode('campaign')}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-bold transition-all cursor-pointer ${creationMode === 'campaign'
                      ? 'bg-emerald-500 text-slate-950 shadow-md shadow-emerald-900/30'
                      : 'text-slate-400 hover:text-white hover:bg-slate-800'
                    }`}
                >
                  <KeyRound className="w-4 h-4" /> Campaign Rep
                </button>
                <button
                  onClick={() => setCreationMode('hub')}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-bold transition-all cursor-pointer ${creationMode === 'hub'
                  ...
                  <form onSubmit={handleCreateInvite} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="e.g. Kazi NGO Field Rep"
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-500"
                      />
                    </div>
                    <div>
                    ...
                    <div>
                      <label className="block text-xs font-medium text-slate-400 mb-1">Assign to Campaign</label>
                      <select
                      ...
                      >
                        ...
                        {campaigns.map(c => <option key={c.campaignId} value={c.campaignId}>{c.name} ({c.district})</option>)}
                      </select>
                    </div>
                    <button
                      type="submit"
                  ...
                  {lastGeneratedInvite && (
                    <div className="bg-gradient-to-br from-emerald-950/60 to-slate-950 border border-emerald-500/40 rounded-2xl p-5 space-y-4 shadow-lg shadow-emerald-900/20 animate-fade-in">
                      ...
                      <div className="text-xs text-slate-400">
                        <span className="text-slate-300">{lastGeneratedInvite.name}</span> → {lastGeneratedInvite.campaignName}
                      </div>
                      {lastGeneratedInvite.email && (
                        <button
                          onClick={handleSendInviteEmail}
                          disabled={emailSending || emailStatus === 'sent'}
                          className={`flex items-center justify-center gap-2 w-full py-2.5 font-bold rounded-xl text-sm transition shadow-lg cursor-pointer ${emailStatus === 'sent'
                  ...
                  <form onSubmit={handleCreateLogisticsInvite} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="logistics.rep@ngo.org"
                    ...
                    <div>
                      ...
                      >
                        ...
                        {Array.isArray(inventory)
                          ? [...new Set(inventory.filter(i => i.warehouseName?.startsWith('[BRAC]')).map(i => i.warehouseName))]
                            .map(hub => <option key={hub} value={hub}>{hub}</option>)
                          : Object.keys(inventory)
                            .filter(k => k.startsWith('[BRAC]'))
                            .map(hub => <option key={hub} value={hub}>{hub}</option>)
                        }
                      </select>
                      <p className="text-[10px] text-slate-500 mt-1">Only your NGO's logistics centers are shown</p>
                    </div>
                    <button
                      type="submit"
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-slate-900 text-slate-400 font-semibold border-b border-slate-800">
                      <tr>
                        ...
                        <th className="p-3">Campaign / Hub</th>
                          ...
                          <td className="p-3 text-slate-300">
                            {inv.assignedHub ? (
                              ...
                              <span className="inline-flex items-center gap-1">
                                ...
                                {inv.campaignName} ({inv.district})
                              </span>
                            )}
                          </td>
                          <td className="p-3">
                            <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${inv.status === 'Registered' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'
                              }`}>
                              {inv.status}
                            </span>
                          </td>
        ...
        {/* NGO VIEW - TAB 3: REQUESTS */}
        {(isNgo && activeTab === 'requests') && (
          <div className="space-y-4">
            <h2 className="text-lg font-bold text-white">Incoming Campaign Resource Requests</h2>
            {requests.length === 0 ? (
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-8 text-center text-slate-400 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {requests.map(r => (
                  <div key={r._id} className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="font-bold text-white text-sm">{r.campaignName || r.assignedHub || r.warehouseName || (r.details && r.details.includes('by ') ? r.details.split('by ')[1] : 'NGO Facility')}</h3>
                        <p className="text-xs text-slate-400">Rep: {r.representativeName || r.submittedByName || r.contactPerson || 'NGO Representative'}</p>
                      </div>
                      <span className={`px-2 py-0.5 text-xs font-semibold rounded ${r.status === 'Approved' ? 'bg-emerald-500/10 text-emerald-400' :
                    ...
                    <p className="text-xs text-slate-300 bg-slate-900 p-2.5 rounded-xl border border-slate-800">
                      <span className="text-slate-500 font-semibold">Needed:</span> {r.itemsNeeded}
                    </p>
                    {r.status === 'Pending' && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-6">
              ...
              <form onSubmit={handleSubmitRepRequest} className="space-y-4">
                ...
                >
                  {actionLoading ? 'Submitting...' : 'Submit Request to NGO'}
                </button>
              </form>
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4">
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  <Boxes className="w-5 h-5 text-emerald-400" /> {mongoUser?.assignedHub ? 'Logistics Hub Live Stock' : 'Campaign Live Stock'}
                </h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <p className="text-xs text-slate-400">Dry Food</p>
                    <p className="text-lg font-bold text-emerald-400">{inventory.dryFood || 0} bags</p>
                  </div>
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <p className="text-xs text-slate-400">Water</p>
                    <p className="text-lg font-bold text-sky-400">{inventory.waterBottles || 0} L</p>
                  </div>
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <p className="text-xs text-slate-400">Medicine</p>
                    <p className="text-lg font-bold text-rose-400">{inventory.medicalKits || 0} kits</p>
                  </div>
                  <div className="bg-slate-900 p-3 rounded-xl border border-slate-800">
                    <p className="text-xs text-slate-400">Shelter Kits</p>
                    <p className="text-lg font-bold text-amber-400">{inventory.shelterKits || 0} packs</p>
                  </div>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    {requests.map(r => (
                      <div key={r._id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex justify-between items-center text-xs">
                        <div>
                          <p className="text-white font-semibold">{r.itemsNeeded}</p>
                          <p className="text-slate-500">Urgency: {r.urgency}</p>
                        </div>
                        <span className={`px-2.5 py-1 font-bold rounded ${r.status === 'Approved' ? 'bg-emerald-500/10 text-emerald-400' :
                            r.status === 'Rejected' ? 'bg-rose-500/10 text-rose-400' : 'bg-amber-500/10 text-amber-400'
                          }`}>
                          {r.status}
                        </span>
              ...
              {/* Village Relief Requests Feed (For Logistics Representatives only) */}
              {(mongoUser?.role === 'NGORepLogistics' || (mongoUser?.role === 'NGORepresentative' && !!mongoUser?.assignedHub)) && (
                <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4">
                  <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            <div className="lg:col-span-3">
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4">
                ...
                {myTransports.length === 0 ? (
                  <div className="py-10 text-center text-slate-500 text-sm">
                    <Package className="w-8 h-8 mx-auto mb-2 opacity-30" />
                    No incoming transports assigned to {mongoUser?.assignedHub || mongoUser?.campaignName || 'you'}.
                  </div>
                ) : (
                  <div className="space-y-3">
                      {myTransports.map(t => {
                          ...
                          <div key={t._id} className="bg-slate-900 rounded-xl border border-slate-800 p-4 flex flex-col sm:flex-row justify-between gap-3">
                            <div className="space-y-1.5 flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {t.loadStatus && (
                                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${t.loadStatus === 'Loaded' ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/20' : 'bg-amber-500/10 text-amber-300 border-amber-500/20'
                                    }`}>
        ...
        {isGov && (
          <div className="space-y-6">
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-4">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-emerald-400" /> Aggregate NGO Campaign Intel (Read-Only)
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                  <p className="text-xs text-slate-400">Total Active Campaigns</p>
                  <p className="text-2xl font-extrabold text-emerald-400">{campaigns.length}</p>
                </div>
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                  <p className="text-xs text-slate-400">Total Campaign Occupancy</p>
                  <p className="text-2xl font-extrabold text-sky-400">
                    {campaigns.reduce((acc, c) => acc + (c.occupancy || 0), 0)} persons
                  </p>
                </div>
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800">
                  <p className="text-xs text-slate-400">Districts Covered</p>
                  <p className="text-2xl font-extrabold text-purple-400">
                    {new Set(campaigns.map(c => c.district)).size} districts
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {campaigns.map(c => (
                <div key={c.campaignId} className="bg-slate-950 border border-slate-800 rounded-2xl p-5 space-y-2 text-xs">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold text-white text-sm">{c.name}</h3>
                    <span className="px-2 py-0.5 bg-slate-800 text-slate-300 rounded">{c.ngoName}</span>
                  </div>
